...
Check memetic algorithm, neural net, selection
remettre output parameters crossover/ size pop/...
...
std::unique_ptr<Method> parse(int argc, const char **argv) {
    ...
    try {
        ...
        cxxopts::Options options(
            argv[0], "Program to launch : mcts, local search or memetic algorithm\n");
        ...
        options.allow_unrecognised_options().add_options()(
            "m,method",
            "method (mcts, local_search, memetic)",
            cxxopts::value<std::string>()->default_value(
                //
                // "mcts"
                // "local_search"
        ...
        options.allow_unrecognised_options().add_options()(
            "n,nb_max_iterations",
            "number of iteration maximum for the mcts or memetic",
        ...
        options.allow_unrecognised_options().add_options()(
            "S,population_size",
            "Size of the population (memetic)",
            cxxopts::value<int>()->default_value("2"));

        options.allow_unrecognised_options().add_options()(
            "L,nb_selected",
            "Number of individuals selected each turns,"
            "each will be assigned a second parent and will create 1 "
            "children (memetic)",
            cxxopts::value<int>()->default_value(
                //
                "2"
        ...
        options.allow_unrecognised_options().add_options()(
            "x,crossover",
            "Crossover for algo mem (to give multiple separate with :)",
            cxxopts::value<std::string>()->default_value(
                //
                // "no_crossover"
                // "gpx"
                // "sum_weights_over_max_weight"
                // "sum_weights_over_max_weight:gpx"
                "gpx"
                //
                ));
        ...
        options.allow_unrecognised_options().add_options()(
            "B,colors_p1",
            "Number of color to pick in the first parent for one color in the second "
            "parent. 1 for balanced crossover, >1 to favor p1",
            cxxopts::value<std::string>()->default_value(
                //
                "1:2:3"
                //
        ...
        options.allow_unrecognised_options().add_options()(
            ...
            cxxopts::value<std::string>()->default_value(
                ...
                // "pursuit"
                "ucb"
                // "neural_net" // take p1 as input
                // "neural_net_cross" // take all possible crossover as input
                //
                ));

        options.allow_unrecognised_options().add_options()(
            "w,window_size",
            "Size of the sliding window for adaptive criteria (for the ones that use it) "
            "actual size = window size * nb_selected",
            cxxopts::value<int>()->default_value("50"));

        options.allow_unrecognised_options().add_options()(
            "l,local_search",
            "Local search selected (to give multiple for memetic algorithm separate with "
            ":)",
            cxxopts::value<std::string>()->default_value(
                //
                // "none"
                ...
                // "ilsts"
                // "ilsts:redls"
                // "ilsts:redls:afisa_original:tabu_weight:tabu_col"
                // "ilsts:redls:afisa_original:tabu_weight"
                "ilsts:redls:afisa_original:tabu_weight:useless_ls:worst_ls"
        ...
        options.allow_unrecognised_options().add_options()(
            "R,return_last_legal",
            "for the local search, true if the local search will return the last legal "
            "found solution, false to return the current solution (unassigned vertices "
            "will stay the same and eventual conflicting vertices will be uncolored)",
        ...
        const int population_size = result["population_size"].as<int>();
        const int nb_selected = result["nb_selected"].as<int>();
        const std::string crossover = result["crossover"].as<std::string>();
        const std::string colors_p1 = result["colors_p1"].as<std::string>();
        ...
        const bool return_last_legal = result["return_last_legal"].as<bool>();
        ...
        fmt::print(stderr,
                   ...
                   "Possible method : mcts, local_search, memetic",
                   method);
        exit(1);
