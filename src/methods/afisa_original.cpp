    ...
    while (not Parameters::p->time_limit_reached_sub_method(max_time) and
        ...
        for (int vertex = 0; vertex < Graph::g->nb_vertices; ++vertex) {
            for (const int &color : possible_colors) {
                ...
                // save best moves
                bool move_not_tabu = true;
                if (color != -1) {
                    move_not_tabu = tabu_matrix.at(vertex).at(color) <= turn_tabu;
                }
                bool better_score = test_score < best_evaluation;
                bool aspiration_criteria = test_score < best_solution.score_wvcp();
                bool zero_penalty = solution.penalty() + delta_penalty == 0;
                if ((better_score and move_not_tabu) or
                    (aspiration_criteria and zero_penalty)) {
                    ...
                    best_evaluation = test_score;
                           ...
                           (move_not_tabu or (aspiration_criteria and zero_penalty))) {
                    best_coloration.emplace_back(Coloration{vertex, color});
                }
            }
        }
