 ...
 * with get_operator(index_solution, solution) -> operator number
 *
 * Then
 * 2. Apply the operator
 * 3. Then give the information of the result to the helper
 * with update_obtained_solution(index_solution, operator_number, score)
 * index_solution is 0 if you feed only one solution to the helper each turn,
 * otherwise i \in [0, Parameter::p->nb_selected] solutions
 * operator_number is the operator used to reach the score
...
class AdaptiveHelper {
  ...
  public:
     ...
     *
     * @param index_solution != 0 if more than one solution is given each turn
...
class AdaptiveHelper_neural_net : public AdaptiveHelper {
  ...
  public:
     ...
     *
     * Don't call this function in parallel !!!
     *
     * @param index_solution 0 if one solution per turn, index otherwise
     ...
     * @return int
     */
    int select_best_child(const std::vector<Solution> &children);
};
