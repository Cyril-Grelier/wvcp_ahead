...
void AdaptiveHelper::update_obtained_solution(const int index_solution,
                                              ...
                                              const int score) {
    const int index =
        ((turn * Parameters::p->nb_selected) % memory_size) + index_solution;
    utility[index] = score;
...
std::string AdaptiveHelper::get_selected_str() const {
    const int index = ((turn * Parameters::p->nb_selected) % memory_size);
    std::string result = "";
    for (int i = 0; i < Parameters::p->nb_selected; ++i) {
        result += std::to_string(past_operators[index + i]) +
                  (i == (Parameters::p->nb_selected - 1) ? "" : ":");
    }
    return result;
}
...
AdaptiveHelper_none::AdaptiveHelper_none(const int nb_operator)
    : AdaptiveHelper(nb_operator, Parameters::p->nb_selected) {
    proba_operator[0] = 1;
    proba_operator[1] = 0;
}
...
void AdaptiveHelper_deleter::update_helper() {
    if ((turn < 5 * nb_operators) or (not(turn % 10 == 0)) or
        (possible_operators.size() == 1)) {
        return;
    }
    // if (possible_operators.size() == 2) {
    //     const int op_0{*possible_operators.begin()};
    //     const int op_1{*(++possible_operators.begin())};
    //     const double mean_op_0{mean_score[op_0]};
    //     const double mean_op_1{mean_score[op_1]};
    //     const double diff{mean_op_0 - mean_op_1};
    //     const double threshold{0.05};
    //     if (diff > 0) {
    //         // see if we delete op 1
    //         const double percentage{mean_op_1 * 100 / mean_op_0};
    //         if (percentage > threshold) {
    //             possible_operators.erase(op_1);
    //             removed_operators.insert(op_1);
    //         }
    //     } else if (diff < 0) {
    //         // see if we delete op 0
    //         const double percentage{mean_op_0 * 100 / mean_op_1};
    //         if (percentage > threshold) {
    //             possible_operators.erase(op_0);
    //             removed_operators.insert(op_0);
    //         }
    //     }
    //     return;
    // }
...
AdaptiveHelper_roulette_wheel::AdaptiveHelper_roulette_wheel(const int nb_operator)
    : AdaptiveHelper(nb_operator,
                     Parameters::p->nb_selected * Parameters::p->window_size) {
}
...
void AdaptiveHelper_ucb::update_helper() {
    ...
    if (turn < memory_size) {
        size = (turn + 1) * Parameters::p->nb_selected;
    }
...
int AdaptiveHelper_neural_net::get_operator(const int index_solution,
    ...
    if (turn < (memory_size / Parameters::p->nb_selected)) {
    ...
    if (distribution(rd::generator) < 20 or turn < 5 * nb_operators) {
        ...
        if (Parameters::p->nb_selected == 1) {
            // update the proba if only one operator to predict
...
void AdaptiveHelper_neural_net::update_helper() {
    if (turn < 5 * nb_operators or turn % 20 != 0) {
        return;
    }
    // train the NN
...
int AdaptiveHelper_neural_net::select_best_child(const std::vector<Solution> &children) {
    int selected;
