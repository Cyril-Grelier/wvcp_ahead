...
int Solution::nb_total_solution = 0;

const std::string Solution::header_csv = "nb_colors,penalty,score,solution";

Solution::Solution() : id(nb_total_solution++), _colors(Graph::g->nb_vertices, -1) {
}
...
int Solution::delete_from_color(const int vertex) {
    ...
    if (_colors_vertices[color].empty()) {
        ...
        // sort the empty colors in reverse
        std::sort(_empty_colors.begin(), _empty_colors.end(), std::greater<>());
    }
